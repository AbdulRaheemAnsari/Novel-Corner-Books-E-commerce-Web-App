...
const FictionProductPage = (props) => {
  ...
  return (
    <>
      <div className="fiction-pageName">
        <p>Fiction</p>
      </div>
      <div className="fiction-wrraper">


        <FilterCategory />


        <div className="fiction-pagemain">
          {currentProducts.map((item) => {
            if (props.categories === item.categories) {
              return (
                <div key={item.id} className="fiction-container">
                  <div className="fiction-cart">
                    <div className="fiction-image">
                      <Link to={`/ProductDisplay/${item.id}`}>
                        <img src={item.img} alt={item.name} />
                      </Link>
                    </div>
                    <div className="fiction-textcontents">
                      <Link to={`/ProductDisplay/${item.id}`}><h1>{item.name}</h1></Link>
                      <h3>By: {item.byowner}</h3>
                      <div className="fiction-reviews">
                        <TiStarFullOutline />
                        <TiStarFullOutline />
                        <TiStarFullOutline />
                        <TiStarFullOutline />
      ...
      <div className="Fictionpagination">
...
export default FictionProductPage